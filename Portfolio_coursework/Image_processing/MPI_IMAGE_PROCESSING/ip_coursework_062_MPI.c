...
/* image details as constants */
#define WIDTH 100 
#define HEIGHT 72
#define IMAGE_SIZE (WIDTH*HEIGHT)
#define BLOCK_SIZE (IMAGE_SIZE/4)

/******************************************************************************
 This program detects edges for the given image using MPI techniques. The program creates 5 
processes. Process with rank 0 is master and others are slave process. 

- Root process sends index marking the beginning of the image to begin processing for edge detection.
- Slave processes upon receiving index calls edge_detection function and obtains processed pixel block into
  a seperate block
- Slave processes sends back the processed image block back to the root process
- the root process reads the sub blocks of pixels into a single results
  and displays the result


  To compile this certain file use:  
    mpicc -o ip_coursework_062_MPI ip_coursework_062_MPI.c -lglut -lGL -lm 
   To run 5 processes on this computer:
    mpirun -n 5 ./ip_coursework_062_MPI
******************************************************************************/
unsigned char results[IMAGE_SIZE];
...
//for calculation of time difference
...
static void key_pressed(unsigned char key, int x, int y) {
 ...
 void main(int argc, char **argv) {
  ...
  int size, rank;
  MPI_Init(NULL, NULL); //initilizing message passing interface
  MPI_Comm_size(MPI_COMM_WORLD, &size); //initilizing the size of message dynamically
  MPI_Comm_rank(MPI_COMM_WORLD, &rank); //process ranking 
  if(size != 5) {
    if(rank == 0) {
    ...
    if(rank ==0){  
             ...
             /* sending of pixel-index of beginning each blocks to 4 different processes */
             ...
             /*receiving processed block back from each process and reading them in results array in order */
             for(int i=1;i<size;i++){
                 MPI_Recv(&results[index], BLOCK_SIZE, MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);  
              	index+=BLOCK_SIZE;
             }
	...
	//initilizing the glute gui library 
  glutInit(&argc, argv);
  glutInitWindowSize(WIDTH * 2,HEIGHT);
  glutInitDisplayMode(GLUT_SINGLE | GLUT_LUMINANCE);
      
  glutCreateWindow("6CS005 Image Progessing Courework using MPI");
  ...
  glutMainLoop(); 
      ...
      if(rank == 1){
              ...
              int starting_index;
              unsigned char sub_result[BLOCK_SIZE];

	      MPI_Recv(&starting_index, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
              
	      /* sending address of the block of image to be processed and reading them into a sub_result block 
               to detect_edges function*/
	      detect_edges(&image[starting_index], sub_result);

              /* sending processed image block back to the master process   */
      	      MPI_Send(&sub_result, BLOCK_SIZE, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
      }
	else if(rank == 2){
