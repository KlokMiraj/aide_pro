...
and compared against past residual error marked as best error.
  To compile:
    nvcc -o lr_coursework_CUDA lr_coursework_CUDA.cu -lm
...
__host__ double residual_error(double x, double y, double m, double c) {
  double e = (m * x) + c - y;
  return e * e;
}
...
__host__ double rms_error(double m, double c) {
  int i;
  double mean;
  double error_sum = 0;
  
  for(i=0; i<DATA_SIZE; i++) {
    error_sum += residual_error(data_set[i].x, data_set[i].y, m, c);
  }
  
  mean = error_sum / DATA_SIZE;
...
__global__ void rms_error_DEVICE(double *m, double *c, double *error_sum_arr, point_t *data_DEVICE) {

	int data_index = threadIdx.x;

  error_sum_arr[data_index] = d_residual_error(data_DEVICE[data_index].x, data_DEVICE[data_index].y, *m, *c);
}
...
int main() {
 ...
 //memory allocation on device using error checking 
/***************************************************/
  if(cudaMalloc(&dm_DEVICE, (sizeof(double) * 8)))
  {
  ...
  if(cudaMalloc(&dc_DEVICE, (sizeof(double) * 8)))
  {
    printf("%s\n", cudaGetErrorString(cudaGetLastError()));
    cudaFree(dm_DEVICE);
    exit(1);
  ...
  if(cudaMalloc(&error_sum_array_DEVICE, (sizeof(double) * 1000)))
  {
     printf("%s\n", cudaGetErrorString(cudaGetLastError()));
     cudaFree(dm_DEVICE);
     cudaFree(dc_DEVICE);
  ...
  if(cudaMalloc(&data_DEVICE, sizeof(data_set)))
  {
    printf("%s\n", cudaGetErrorString(cudaGetLastError()));
    cudaFree(dm_DEVICE);
    cudaFree(dc_DEVICE);
    cudaFree(error_sum_array_DEVICE);
    exit(1);
  } 
  

 /*********************copying host dataset to device****************************/

  if(cudaMemcpy(data_DEVICE, data_set, sizeof(data_set), cudaMemcpyHostToDevice))
  {
    printf("%s\n", cudaGetErrorString(cudaGetLastError()));
    exit(1);
  }
  ...
  while(!minimum_found) {
  	...
  	if(cudaMemcpy(dm_DEVICE, dm, (sizeof(double) * 8), cudaMemcpyHostToDevice))  
  	{
	printf("%s\n", cudaGetErrorString(cudaGetLastError()));
	exit(1);
	}
		
  	if(cudaMemcpy(dc_DEVICE, dc, (sizeof(double) * 8), cudaMemcpyHostToDevice))  
  	{
	printf("%s\n", cudaGetErrorString(cudaGetLastError()));
	exit(1);
	}
       ...
       for(i=0;i<8;i++) {
		...
		double error_sum_array_HOST[1000];
		double error_sum_total;
		double error_sum_mean;
		rms_error_DEVICE <<<1,1000>>>(&dm_DEVICE[i], &dc_DEVICE[i], error_sum_array_DEVICE, data_DEVICE);
		cudaThreadSynchronize();
		
                if(cudaMemcpy(&error_sum_array_HOST, error_sum_array_DEVICE, (sizeof(double) * 1000), cudaMemcpyDeviceToHost))  
		 {
		printf("%s\n", cudaGetErrorString(cudaGetLastError()));
		exit(1);
		}
		...
		for(int j=0; j<DATA_SIZE; j++) {
    		error_sum_total += error_sum_array_HOST[j];
  		}

		error_sum_mean = error_sum_total / DATA_SIZE;
...
/************************freeing device memory used during the computation*****************************/

  if(cudaFree(dm_DEVICE))
  {
     printf("%s\n", cudaGetErrorString(cudaGetLastError()));
     exit(1);
  }

  if(cudaFree(dc_DEVICE))
  {
     printf("%s\n", cudaGetErrorString(cudaGetLastError()));
     exit(1);
  }

  if(cudaFree(data_DEVICE))
  {
     printf("%s\n", cudaGetErrorString(cudaGetLastError()));
     exit(1);
  }
		
  if(cudaFree(error_sum_array_DEVICE))
  {
     printf("%s\n", cudaGetErrorString(cudaGetLastError()));
     exit(1);
  }
